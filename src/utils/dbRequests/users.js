import { doc, getDocs, setDoc,collection, where, updateDoc, query, deleteDoc } from "firebase/firestore";
...
export const getUserFromEmail = async (email) => {
  ...
  export const saveUser = async (user) => {
    await withErrorHandling(async () => {
      const id = user.name.toLowerCase().replace(" ", "_") + Date.now().toString();    
      user.id = id;
